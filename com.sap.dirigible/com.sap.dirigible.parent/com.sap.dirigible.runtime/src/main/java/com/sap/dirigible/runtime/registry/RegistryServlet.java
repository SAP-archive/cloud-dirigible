...
public class RegistryServlet extends AbstractRegistryServlet {
    ...
    private static final Logger log = LoggerFactory
            .getLogger(RegistryServlet.class.getCanonicalName());
            ...
            if (entity != null) {
                if (entity instanceof IResource) {
                	...
                	if (acceptHeader != null
                		...
                		if (index.exists() 
                				&& (collectionPath.endsWith(IRepository.SEPARATOR))) {
    ...
    private void exceptionHandler(final HttpServletResponse response, final String repositoryPath, final int errorMessage, final String exceptionMessage)
            throws IOException {
        log.error(String
                .format(REQUEST_PROCESSING_FAILED_S, repositoryPath) + exceptionMessage);
        ...
        for (final String collectionName : collections) {
            ...
            if (deep && isFolder) {
                final JsonArray children =
                    enumerateCollectionData(path, collection.getCollection(collectionName), deep);
