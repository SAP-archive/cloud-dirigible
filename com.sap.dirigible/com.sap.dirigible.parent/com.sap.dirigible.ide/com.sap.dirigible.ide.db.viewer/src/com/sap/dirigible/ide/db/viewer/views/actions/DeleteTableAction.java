...
public class DeleteTableAction extends Action {
	...
	private static final Logger logger = Logger
			.getLogger(DeleteTableAction.class);
	...
	public void run() {
		...
		if (TreeObject.class.isInstance(obj)) {
			if (((TreeObject) obj).getTableDefinition() != null) {
				...
				IDbConnectionFactory connectionFactory = parent
						.getConnectionFactory();
				try {
					deleteTable(tableDefinition, connectionFactory);
					RefreshViewAction refresh = new RefreshViewAction(viewer,
							parent);
					refresh.run();
	...
	protected void showMessage(String message) {
		MessageDialog.openError(viewer.getControl().getShell(), DATABASE_VIEW,
				message);
	}

	private void deleteTable(TableDefinition tableDefinition,
			IDbConnectionFactory connectionFactory) throws SQLException {
